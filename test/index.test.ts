...
describe("My Probot app", () => {
  ...
  test("creates a comment when an issue is opened", async () => {
    const mock = nock("https://api.github.com")
      ...
      // Test that a comment is posted on an issue
      .post("/repos/hiimbex/testing-things/issues/1/comments", (body: any) => {
        expect(body).toMatchObject(issueCreatedBody);
        return true;
      })
    ...
    // Receive a webhook event for issue opened
  ...
  test("creates a comment when a pull request is opened", async () => {
    const mock = nock("https://api.github.com")
      ...
      // Test that a comment is posted on a pull request
      .post("/repos/hiimbex/testing-things/issues/1/comments", (body: any) => {
        expect(body).toMatchObject(pullRequestCommentBody);
        return true;
      })
      .reply(200);
